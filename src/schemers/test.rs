...
mod parser_tests {
    ...
    fn can_parse_boolean_literals() {
        let false_atom = read("#f".to_string());
        assert_eq!(false_atom, Atom(Boolean(false)));
        let true_atom = read("#t".to_string());
        assert_eq!(true_atom, Atom(Boolean(true)));
        let false_atom = read("#false".to_string());
        assert_eq!(false_atom, Atom(Boolean(false)));
        let true_atom = read("#true".to_string());
        assert_eq!(true_atom, Atom(Boolean(true)));
    }
}
...
mod eval_tests {
    ...
    fn given_a_float_literal_then_calling_eval_should_return_it_back() {
        let env = Env::new(None, None, None);
        let in_expr = read("34.3".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn given_a_quote_of_an_atom_expr_should_resolve_to_just_the_atom() {
        let env = Env::new(None, None, None);
        let in_expr = read("(quote 42)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn a_quote_with_no_params_should_fail() {
        let env = Env::new(None, None, None);
        let in_expr = read("(quote)".to_string());
        eval(in_expr, env);
    }
    ...
    fn given_a_quote_of_a_list_expr_it_should_resolve_to_the_list_without_resolving_symbols() {
        let env = Env::new(None, None, None);
        let in_expr = read("(quote (x 37))".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn only_an_empty_list_expr_should_have_is_null_return_true() {
        let empty_list = read("()".to_string());
        assert_eq!(empty_list.is_null(), true);
        let not_null = read("(())".to_string());
        assert_eq!(not_null.is_null(), false);
        let not_null = read("2".to_string());
        assert_eq!(not_null.is_null(), false);
        let not_null = read("x".to_string());
        assert_eq!(not_null.is_null(), false);
        let not_null = read("34.4".to_string());
        assert_eq!(not_null.is_null(), false);
        let not_null = read("(x 3 4)".to_string());
        assert_eq!(not_null.is_null(), false);
    }
    ...
    fn an_if_call_with_a_non_false_test_should_resolve_the_conseq_branch() {
        ...
        let (out_expr, env) =
            eval(read("(if (quote 1)(quote conseq) (quote alt))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Symbol("conseq".to_string())));
        let (out_expr, env) =
            eval(read("(if #t (quote conseq) (quote alt))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Symbol("conseq".to_string())));
        let (out_expr, _) =
            eval(read("(if (list)(quote conseq) (quote alt))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Symbol("conseq".to_string())));
    }
    ...
    fn an_if_call_with_a_false_test_should_resolve_the_alt_branch() {
        let env = Env::new(None, None, None);
        let in_expr = read("(if #f (quote conseq) (quote alt))".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn calling_set_on_a_var_not_in_the_scope_should_fail() {
        let env = Env::new(None, None, None);
        let in_expr = read("(set! x 123)".to_string());
        eval(in_expr, env);
    }
    ...
    fn calling_set_should_resolve_the_val() {
        let env = Env::new(
            Some(vec!("x".to_string())), Some(vec!(Atom(Number::integer(42)))),
            None);
        let in_expr = read("(set! x (quote 37))".to_string());
        let (_, env) = eval(in_expr, env);
    ...
    fn calling_set_on_a_var_in_an_outer_scope_should_succeed() {
        ...
        let env = Env::new(
            None, None,
            Some(outer_env));
        let in_expr = read("(set! x 43)".to_string());
        let (_, env) = eval(in_expr, env);
    ...
    fn calling_set_with_a_non_symbol_as_the_atom_should_fail() {
        let env = Env::new(None, None, None);
        let in_expr = read("(set! 1 123)".to_string());
        eval(in_expr, env);
    }
    ...
    fn an_if_test_that_returns_a_None_out_expr_should_run_the_conseq_branch() {
        let env = Env::new(
            Some(vec!("x".to_string())), Some(vec!(Atom(Number::integer(42)))),
            None);
        let in_expr = read("(if (set! x 37) (quote conseq) (quote alt))".to_string());
    ...
    fn evaling_a_define_should_return_None() {
        let env = Env::new(None, None, None);
        let in_expr = read("(define x 123)".to_string());
        let (result, _) = eval(in_expr, env);
    ...
    fn calling_define_should_resolve_the_val_of_the_input() {
        let env = Env::new(None, None, None);
        let in_expr = read("(define x (quote foo))".to_string());
        let (_, env) = eval(in_expr, env);
    ...
    fn defining_a_var_already_set_in_an_outer_scope_should_shadow_it_in_the_inner_scope() {
        ...
        let in_expr = read("(define x 37)".to_string());
        let (_, env) = eval(in_expr, env);
        assert_eq!(env.find(&"x".to_string()), Atom(Number::integer(37)));
    ...
    fn lambda_exprs_should_expect_a_list_for_vars_and_an_expr_for_body() {
        let env = Env::new(None, None, None);
        let in_expr = read("(lambda (x) 37)".to_string());
        let (out_expr, _) = eval(in_expr, env);
        match out_expr.unwrap() {
            Atom(Lambda(body)) => {
                match body {
    ...
    fn lambda_exprs_evald_outside_of_define_have_the_name_of_anonymous() {
        let env = Env::new(None, None, None);
        let in_expr = read("(lambda (x y) 37)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn lambda_exprs_evald_within_a_define_have_the_of_the_provided_var_symbol() {
        let env = Env::new(None, None, None);
        let in_expr = read("(define foo (lambda (x y) 37))".to_string());
        let (_, env) = eval(in_expr, env);
    ...
    fn lambda_expr_eval_should_fail_if_a_list_isnt_in_the_vars_position() {
        let env = Env::new(None, None, None);
        let in_expr = read("(define foo (lambda x 37))".to_string());
        eval(in_expr, env);
    }
    #[test]
    #[should_fail]
    fn lambda_expr_eval_should_fail_if_theres_no_third_position() {
        let env = Env::new(None, None, None);
        let in_expr = read("(define foo (lambda (x)))".to_string());
        eval(in_expr, env);
    }
    ...
    fn begin_should_return_the_evald_atom_expr_of_the_tail_param() {
        let env = Env::new(None, None, None);
        let in_expr = read("(begin (define x 37) 42)".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(env.find(&"x".to_string()), Atom(Number::integer(37)));
        assert_eq!(out_expr.unwrap(), Atom(Number::integer(42)));
    }
    ...
    fn begin_should_return_no_expr_if_its_tail_param_returns_nothing() {
        let env = Env::new(None, None, None);
        let in_expr = read("(begin (define x 37))".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(env.find(&"x".to_string()), Atom(Number::integer(37)));
    ...
    fn begin_can_have_a_list_in_the_tail_position() {
        let env = Env::new(None, None, None);
        let in_expr = read("(begin (define x 37) (quote (1 x 3.4)))".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(env.find(&"x".to_string()), Atom(Number::integer(37)));
        match out_expr.unwrap() {
            List(items) => {
    ...
    fn can_define_a_var_with_the_same_name_as_a_special_form_keyword_but_SF_is_still_usable() {
        let env = Env::new(None, None, None);
        let in_expr = read("(begin (define set! 42) (set! set! 37) set!)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn can_define_a_lambda_that_returns_an_atom_and_subsequently_invoke_it() {
        let env = Env::new(None, None, None);
        let in_expr = read("(define get-1 (lambda () 1))".to_string());
        let (_, env) = eval(in_expr, env);
        let in_expr = read("(get-1)".to_string());
        let (out_expr, _) = eval(in_expr, env);
...
mod builtins_tests {
    ...
    fn two_plus_two_equals_four() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(+ 2 2)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn can_sum_an_arbitrary_number_of_items() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(+ 2 2 1 1 1 1 4)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn two_point_five_plus_two_equals_four_point_five() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(+ 2.5 2)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn four_minus_three_equals_one() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(- 4 3)".to_string());
        let (out_expr, _) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Number::integer(1)));
    }
    #[test]
    fn four_point_five_minus_one_equals_equals_three_point_five() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(- 4.5 1)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn can_subtract_an_arbitrary_number_of_items() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(- 4.5 1 3)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn two_times_twenty_one_equals_42() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(* 2 21)".to_string());
        let (out_expr, _) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Number::integer(42)));
    }
    #[test]
    fn can_multiply_an_aribtrary_number_of_items() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(* 4 5 3)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn two_times_one_point_five_equals_three() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(* 2 1.5)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn ten_divided_by_5_equals_2() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(/ 10 5)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn five_divided_by_two_point_five_equals_2() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(/ 5 2.5)".to_string());
        let (out_expr, _) = eval(in_expr, env);
    ...
    fn can_divide_an_arbitrary_number_of_items() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(/ 100 10 5)".to_string());
        let (out_expr, _) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Number::integer(2)));
    }
    #[test]
    fn lt_works() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(< 2 4)".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let in_expr = read("(< 5 1)".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let in_expr = read("(< 1.1 1.2)".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let in_expr = read("(< 1.1 2)".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let in_expr = read("(< 1.0 1)".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let in_expr = read("(< 1 1)".to_string());
        let (out_expr, _) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
    }
    #[test]
    fn gt_works() {
        let env = add_builtins(Env::new(None, None, None));
        let in_expr = read("(> 4 2)".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let in_expr = read("(> 1 5)".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let in_expr = read("(> 1.2 1.1)".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let in_expr = read("(> 2 1.1)".to_string());
        let (out_expr,env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let in_expr = read("(not #f)".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let in_expr = read("(not (quote #f))".to_string());
        let (out_expr, env) = eval(in_expr, env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let in_expr = read("(> 1.0 1)".to_string());
        let (out_expr, env) = eval(in_expr, env);
    ...
    fn a_call_to_not_with_anything_that_isnt_false_returns_false() {
        let env = add_builtins(Env::new(None, None, None));
        let (out_expr, env) = eval(read("(not #t)".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let (out_expr, env) = eval(read("(not (quote ()))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let (out_expr, env) = eval(read("(not 1)".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let (out_expr, env) = eval(read("(not 1.3)".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let (out_expr, env) = eval(read("(not (quote 1))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let (out_expr, env) = eval(read("(not (lambda (x) x))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let (out_expr, _) = eval(read("(not (+ 1 1))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
    }
    #[test]
    fn gte_works() {
        let env = add_builtins(Env::new(None, None, None));
        let (out_expr, env) = eval(read("(>= 2 2)".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let (out_expr, env) = eval(read("(>= 2 2.0)".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let (out_expr, env) = eval(read("(>= 2 1.9)".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let (out_expr, env) = eval(read("(>= 1.9 2)".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let (out_expr, _) = eval(read("(>= 1 2)".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
    }
    #[test]
    fn lte_works() {
    ...
    #[test]
    fn equal_works() {
        let (out_expr, _) = eval(read("(= 1 1)".to_string()),
                                 add_builtins(Env::new(None, None, None)));
    ...
    fn cons_should_fail_with_one_arg() {
        eval(read("(cons 1)".to_string()),
             add_builtins(Env::new(None, None, None)));
    }
    #[test]
    #[should_fail]
    fn cons_should_fail_with_more_than_two_args() {
        eval(read("(cons 1 2 3)".to_string()),
             add_builtins(Env::new(None, None, None)));
    }
    #[test]
    fn car_should_pull_the_first_element_from_a_list() {
        let (out_expr, _) = eval(read("(car (quote (1 2 3)))".to_string()),
                                 add_builtins(Env::new(None, None, None)));
    ...
    fn cdr_should_fail_when_applied_to_an_atom_value() {
        eval(read("(cdr 1)".to_string()),
             add_builtins(Env::new(None, None, None)));
    }
    #[test]
    fn append_adds_an_expr_to_the_end_of_a_list() {
        let (out_expr, _) = eval(read("(append (quote (1)) 2)".to_string()),
                                 add_builtins(Env::new(None, None, None)));
        assert_eq!(out_expr.unwrap().print(), "(1 2)".to_string());
    }
    #[test]
    ...
    fn append_should_fail_if_the_first_arg_isnt_a_list() {
        eval(read("(append 1 2".to_string()),
             add_builtins(Env::new(None, None, None)));
    }
    #[test]
    #[should_fail]
    fn append_should_fail_when_passed_one_arg() {
        eval(read("(append (quote (1)))".to_string()),
             add_builtins(Env::new(None, None, None)));
    }
    #[test]
    #[should_fail]
    fn append_should_fail_with_more_than_two_args() {
        eval(read("(append (quote (1)) 1 2)".to_string()),
             add_builtins(Env::new(None, None, None)));
    }
    #[test]
    fn applying_list_with_zero_args_should_return_an_empty_list() {
        let (out_expr, _) = eval(read("(list)".to_string()),
                                 add_builtins(Env::new(None, None, None)));
        assert_eq!(out_expr.unwrap().is_null(), true);
    }
    #[test]
    fn applying_list_to_any_number_of_arguments_returns_a_list_of_those_arguments() {
        let (out_expr, env) = eval(read("(list 1 2 3)".to_string()),
                                   add_builtins(Env::new(None, None, None)));
        assert_eq!(out_expr.unwrap().print(), "(1 2 3)".to_string());
        let (out_expr, _) = eval(read("(list 1 (list 2) 3)".to_string()), env);
        assert_eq!(out_expr.unwrap().print(), "(1 (2) 3)".to_string());
    }
    #[test]
    fn list_predicate_returns_false_for_atom_values() {
        let (out_expr, _) = eval(read("(list? 1)".to_string()),
                                 add_builtins(Env::new(None, None, None)));
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
    }
    #[test]
    fn list_predicate_returns_true_for_any_list_value() {
        let (out_expr, env) = eval(read("(list? (list))".to_string()),
                                   add_builtins(Env::new(None, None, None)));
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let (out_expr, _) = eval(read("(list? (list 1 2 3))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
    }
    #[test]
    fn null_predicate_returns_true_only_for_empty_list_values() {
        let (out_expr, env) = eval(read("(null? (list))".to_string()),
                                   add_builtins(Env::new(None, None, None)));
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let (out_expr, env) = eval(read("(null? (quote x))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let (out_expr, _) = eval(read("(null? (list 1 2 3))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
    }
    #[test]
    fn symbol_predicate_returns_true_only_for_empty_list_values() {
        let (out_expr, env) = eval(read("(symbol? (list))".to_string()),
                                   add_builtins(Env::new(None, None, None)));
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
        let (out_expr, env) = eval(read("(symbol? (quote x))".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(true)));
        let (out_expr, _) = eval(read("(symbol? 1)".to_string()), env);
        assert_eq!(out_expr.unwrap(), Atom(Boolean(false)));
    }
...
000000000".to_string());
    ...
    fn syntactic_keyword_in_lambda_doesnt_choke() {
        ...
        let (_, env) = eval(read(eight),
                            add_builtins(Env::new(None, None, None)));
        let (out_expr, _) = eval(read("(foo (list 1 2 3))".to_string()), env);
        assert_eq!(out_expr.unwrap().print(), "(1 2 3)".to_string());
    }
    #[test]
